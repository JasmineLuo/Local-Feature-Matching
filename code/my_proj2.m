...
% handle harder cases.
% --------Notre Dame-------
  image1 = imread('921919841_a30df938f2_o.jpg');
  image2 = imread('4191453057_c86028ce1f_o.jpg');
  eval_file = '921919841_a30df938f2_o_to_4191453057_c86028ce1f_o.mat';

% --------Mont Rushmore----
% image1=imread('9021235130_7c2acd9554_o.jpg');
% image2=imread('9318872612_a255c874fb_o.jpg');
% eval_file = '9021235130_7c2acd9554_o_to_9318872612_a255c874fb_o.mat';

%This pair is relatively difficult
% image1 = imread('3872126052_0e08c5a830_o.jpg');
% image2 = imread('3872129062_9a1b8c98d7_o.jpg');
 %eval_file = '4386465943_8cf9776378_o_to_3743214471_1b5bbfda98_o.mat';
 
 %This pair is relatively difficult
%  image1 = imread('4872728253_3e73bd436b_o.jpg');
%  image2 = imread('6005180348_c465ccf905_o.jpg');
...
feature_width = 30; %width and height of each local feature, in pixels. 
                    %--------------------------------------origin 30

%% Find distinctive points in each image. Szeliski 4.1.1
% !!! You will need to implement get_interest_points. !!!
[x1, y1] = get_interest_points2(image1_bw, feature_width);  %%647 found
[x2, y2] = get_interest_points2(image2_bw, feature_width);  

%% Create feature vectors at each interest point. Szeliski 4.1.2
% !!! You will need to implement get_features. !!!
[image1_features,xc1,yc1] = get_features(image1_bw, x1, y1, feature_width);
[image2_features,xc2,yc2] = get_features(image2_bw, x2, y2, feature_width);

%save('features.mat','image1_features','image2_features','xc1','yc1','xc2','yc2'); %%-----for debug

%% Match features. Szeliski 4.1.3
% !!! You will need to implement get_features. !!!
[matches, confidences] = match_features(image1_features, image2_features, xc1, yc1, xc2, yc2);
...
%% for debug in the third
 [C,IND]=sort(confidences,'descend');
 temp=matches;
 for k=1:1:min(length(confidences),100)
     temp(k,:)=matches(IND(k),:);
 end
 matches=temp;
...
%put confidence in order
%% END

% There are two visualization functions. You can comment out one of both of
% them if you prefer.
num_pts_to_visualize = min(size(matches,1),100);
show_correspondence(image1, image2, xc1(matches(1:num_pts_to_visualize,1)), ...
                                    yc1(matches(1:num_pts_to_visualize,1)), ...
                                    xc2(matches(1:num_pts_to_visualize,2)), ...
                                    yc2(matches(1:num_pts_to_visualize,2)));
                                 
show_correspondence2(image1, image2, xc1(matches(1:num_pts_to_visualize,1)), ...
                                     yc1(matches(1:num_pts_to_visualize,1)), ...
                                     xc2(matches(1:num_pts_to_visualize,2)), ...
                                     yc2(matches(1:num_pts_to_visualize,2)));
...
evaluate_correspondence(image1, image2, eval_file, scale_factor, ... 
                        xc1(matches(1:num_pts_to_evaluate,1)), ...
                        yc1(matches(1:num_pts_to_evaluate,1)), ...
                        xc2(matches(1:num_pts_to_evaluate,2)), ...
                        yc2(matches(1:num_pts_to_evaluate,2)));
